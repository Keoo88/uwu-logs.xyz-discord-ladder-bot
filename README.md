...
- 🌐 **Несколько серверов:** команды можно регистрировать сразу на нескольких Discord-серверах.
...
| `DISCORD_GUILD_ID` | — | ID сервера(ов) для мгновенной регистрации команд. Несколько — через запятую (`111,222`). Пусто → глобально (до 1 часа) |
| `WARMANE_GUILD` | `RUnion` | Название гильдии (только для заголовка ладдера) |
| `WARMANE_REALM` | `Icecrown` | Реалм (только для заголовка) |
| `UWU_SERVER` | `Icecrown` | Имя реалма для запросов к UwU Logs |
